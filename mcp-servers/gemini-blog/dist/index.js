...
const server = new Server({ name: "gemini-blog", version: "1.0.0" }, { capabilities: { tools: {} } });
server.setRequestHandler(ListToolsRequestSchema, async () => ({
    tools: [
        {
            name: "gemini_research",
            description: "Gemini + Google Search でブログ記事のキーワード調査を行う。対象キーワード・検索意図・差別化ポイント・推奨構成・内部リンク候補を返す。",
            inputSchema: {
                type: "object",
                properties: {
                    topic: {
                    ...
                    existing_articles: {
                        type: "string",
                        description: "既存記事のスラッグをカンマ区切りで（重複回避用）。例: life-game-banker,life-game-rules",
                    },
                },
                required: ["topic"],
            },
        ...
        {
            name: "gemini_review",
            description: "Gemini でブログ原稿を推敲する。文字数・構成・CTAの自然さ・アプリ機能の正確さ・SEO観点でフィードバックを返す。",
            inputSchema: {
                type: "object",
                properties: {
                    draft_content: {
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    if (name === "gemini_research") {
        ...
        const existingArticles = args?.existing_articles ?? "";
